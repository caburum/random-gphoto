<script lang="ts">
	...
	import { currentRandomMediaItemPromise, mediaCount } from './lib/stores';
...
{#if $authState.id}
	{#if $mediaCount > 0}
		...
		{:catch error}
			<p>{error?.message}</p>
		{/await}
	{:else if $mediaCount !== undefined}
		<p>no media items</p>
		<p>go to <a href="#/settings">settings</a> and click "update media" to load your photos</p>
	{/if}
{/if}
